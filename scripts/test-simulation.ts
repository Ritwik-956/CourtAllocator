...
console.log('--- Running Court Allocator Simulation Tests ---');
...
function test3WinSplit() {
  ...
  // Players in the pool
  const selectedPlayers = ['GAURI', 'SAIKANTH', 'MONIT', 'PRACHEE', 'RITWIK', 'ANNA', 'JOHN', 'DOE'];
  const sitOutCount = {}; // No one has priority over anyone else for sitting out
  const courtCount = 2; // Needs 8 players

  const { nextCourts, nextWinStreaks, remainingChallengers } = computeNextRoundState({
    seeds,
    courts,
    winners,
    winStreaks,
  ...
  for (const court of nextCourts) {
    const isPairedT1 = court.team1.includes('GAURI') && court.team1.includes('SAIKANTH');
    const isPairedT2 = court.team2.includes('GAURI') && court.team2.includes('SAIKANTH');
    if (isPairedT1 || isPairedT2) {
      areTheyPaired = true;
    }
  }

  assert(!areTheyPaired, 'GAURI and SAIKANTH should be split up because of their 3-win streak');
  console.log('✅ Passed: 3-Win players are successfully split up and not paired together');
}
...
function testStarvationPrevention() {
  ...
  const selectedPlayers = ['PLAYER_A', 'PLAYER_B', 'PLAYER_C', 'PLAYER_D', 'STARVED_1', 'STARVED_2'];
  ...
  assert(!remainingChallengers.includes('STARVED_1'), 'STARVED_1 must be prioritized and should not sit out.');
  assert(!remainingChallengers.includes('STARVED_2'), 'STARVED_2 must be prioritized and should not sit out.');
  
  console.log('✅ Passed: Starvation mapping strictly prioritizes players with highest sit-out rate.');
}
